...
namespace Populate_Required_Date
{
    public class Populate_Required_Date : P21.Extensions.BusinessRule.Rule
    {
        ...
        private static string BuildSalesOrderSql(int invMastUidCount)
        {
            ...
            sql.AppendLine("        , CAST(");
            sql.AppendLine("              CASE");
            sql.AppendLine("                  WHEN ISNULL(oel.scheduled, 'N') = 'Y' THEN next_schedule.release_date");
            sql.AppendLine("                  ELSE oel.required_date");
            sql.AppendLine("              END AS date");
            sql.AppendLine("          ) AS required_date");
            sql.AppendLine("        , CAST(");
            sql.AppendLine("              CASE");
            sql.AppendLine("                  WHEN ISNULL(oel.scheduled, 'N') = 'Y' THEN next_schedule.release_qty");
            sql.AppendLine("                  ELSE oel.qty_ordered");
            sql.AppendLine("              END AS decimal(19, 6)");
            sql.AppendLine("          ) AS qty_ordered");
            ...
            sql.AppendLine("              oels.release_no");
            sql.AppendLine("            , oels.oe_line_schedule_uid");
            sql.AppendLine("            , oels.release_date");
            sql.AppendLine("            , oels.release_qty");
            sql.AppendLine("        FROM oe_line_schedule oels");
            sql.AppendLine("        WHERE oels.order_no = oel.order_no");
            sql.AppendLine("          AND oels.line_no = oel.line_no");
            sql.AppendLine("          AND CAST(oels.release_date AS date) >= CAST(GETDATE() AS date)");
            sql.AppendLine("          AND oels.allocated_qty <= 0");
            sql.AppendLine("          AND ISNULL(oels.qty_picked, 0) <= 0");
            sql.AppendLine("        ORDER BY");
            sql.AppendLine("              oels.release_date ASC");
            sql.AppendLine("            , oels.release_no ASC");
            sql.AppendLine("            , oels.oe_line_schedule_uid ASC");
            sql.AppendLine("    ) next_schedule");
            ...
            sql.AppendLine("              AND next_schedule.release_date IS NOT NULL");
            sql.AppendLine("          )");
            sql.AppendLine("          OR");
            sql.AppendLine("          (");
            sql.AppendLine("              ISNULL(oel.scheduled, 'N') <> 'Y'");
